...
from .serializers import CollectUserDetailedSerializer
from .serializers import CollectUserSkippedProfiles
from .serializers import CollectUserToDashboardSerializer
from .serializers import EditProfilePIcSerializer
from .serializers import GetUserImageOne
from .serializers import LikedProfileSerializer
from .serializers import MessageSerializer
from .serializers import PartnerPrefenceSerializer
from .serializers import PassedProfileSerializer
from .serializers import RegisterSerializer
from .serializers import SubscribedusersSerializer
from .serializers import UpdatePartnerPreferenceSerializer
from .serializers import UserEditSerializer
from .serializers import UserFamilyEditSerializer
from .serializers import UserFamilySerializer
from .serializers import UserImageSerializer
from .serializers import UserLocationEditSerializer
from .serializers import UserLocationSerializer
from .serializers import UserProfessionEditSerializer
from .serializers import UserProfessionSerializer
...
class ViewUser(APIView):
    ...
    def get(self, request, format=None):
        ...
        data = {"name": request.user.full_name, "image": userImage.data}
...
class UserLocationViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(
            ...
            register=LogedInUser.register,
            user_profile=LogedInUser.userprofile,
            user_profession=LogedInUser.userprofession,
            user_family=LogedInUser.userfamily,
        )
...
class UserImageViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        serializer.save(
            ...
            register=LogedInUser.register,
            user_profile=LogedInUser.userprofile,
            user_location=LogedInUser.userlocation,
            user_profession=LogedInUser.userprofession,
            user_family=LogedInUser.userfamily,
        )
        return Response(status=status.HTTP_204_NO_CONTENT)
    ...
    def get(self, request):
        ...
        if queryset.exists():
            ...
            if querysetIdGet:
                ...
                querysetId = 0
        return Response({"partnerpreference": queryset.exists(), "id": querysetId})
...
class Payment(APIView):
    ...
    def get(self, request, format=None):
        ...
        if Subscription.objects.filter(id=subscriptionid).exists():
            ...
            order = SubscripedUser.objects.create(amount=amount, razorpay_order_id=payment["id"], user=user, subscription=subscriptiondata, subscription_end_date=end_date)
...
class SubscribedUserView(APIView):
    ...
    def post(self, request, format=None):
        ...
        data = {"razorpay_order_id": ord_id, "razorpay_payment_id": raz_pay_id, "razorpay_signature": raz_signature, "user": self.request.user}

        rupees = SubscripedUser.objects.get(razorpay_order_id=ord_id).amount * 100
        ...
        if check is True:
            amount = rupees
            client.payment.capture(raz_pay_id, amount)
            order.status = "Paid"
            order.save()
